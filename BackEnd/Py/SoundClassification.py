...
def load_validated_data(tsv_file):
    ...
    # filter conditions
    filtered_df = df[
        df['gender'].notna() & (df['gender'] != '') &                  # Gender is not empty
        (df['gender'] != 'other') &                                    # Exclude 'other'
        (df['up_votes'] > 3) &                                         # up_votes is greater than 3
        (df['down_votes'] == 0) &                                      # down_votes equals 0
        df['age'].notna() & (df['age'] != '') &                        # age is not empty
        df['accent'].notna() & (df['accent'] != '')                    # accent is not empty
    ]
...
def train_model(tsv_file):
    ...
    if num_outputs == 6:  # Includes loss and accuracy
...
if __name__ == "__main__":
    ...
    if args.mode == 'fine-tune':
        ...
        # Load encoder
        le_accent = joblib.load('Py/le_accent.pkl')
        le_age = joblib.load('Py/le_age.pkl')
        le_gender = joblib.load('Py/le_gender.pkl')
        encoders = (le_accent, le_age, le_gender)
